...
set(0,'DefaultFigureVisible','on')             %Change 'off' to 'on' if you want plotted figures to show, also refer to the last line of code (295)
...
for k = 1 : numberOfFolders
    ...
    Summary_Data = zeros(numberOfEDFFiles,3);       % Saves the mouse spikes/discharges for export to excel
...
%Reorganize Data for Hourly Counts ----------------------------------------------------------------
            ...
            filename = strcat(Mouse_Number,' EEG Spike and Discharge Data by Hour.xlsx');     % Generates the name of the excel file using mouse number
            writetable(T_out,filename,'Sheet',1,'Range','B2')               % Exports data and creates excel file with discharge data
            
            % Clean up data for discharge detection --------------------------------------------------
...
% Discharge Detector -----------------------------------------------------------------------------
            ...
            for r = 1:length(Interval)
                if Interval(1,r) > 10                                   % Loop exlcudes spikes greater than 10 seconds apart for discharges
                    Status(1,r) = 0;                                    % If true (> 10 seconds apart), the discharge chain breaks
                    Status(2,r) = Interval(2,r);                        % Saves the legnth of time between spikes
                else
                    Status(1,r) = 1;                                    % If false (< 10 seconds apart), the discharge chain continues
                    Status(2,r) = Interval(2,r);                        % Saves the lengths of time between spikes
                end
            end
            
            Discharge_Record = zeros(length(Status),5);                 % Placeholder variable
            Spike_Tally = 0;                                            % Placeholder variable
            Discharge_Length = 0;                                       % Placeholder variable
           
            for s = 1:length(Status)     
                if Status(1,s) == 1                                     % If the spikes are less than 10 seconds apart, the loop continues
                    Spike_Tally = Spike_Tally + 1;                      % Counts the number of spikes in each discharge
                    Discharge_Length = Discharge_Length + Interval(1,s);% Records the growing length of the discharge
                else                                                    % If spikes are greater than 10 seconds apart, the loop breaks
                    Spike_Tally = Spike_Tally + 1;                      % Memory variable. DO NOT CHANGE!
                    Discharge_Record(s,1) = Spike_Tally;                % Saves the number of spikes in the last discharge
                    Discharge_Record(s,2) = Discharge_Length;           % Saves the length of the last discharge
                    Discharge_Record(s,3) = Interval(2,s-Spike_Tally+1);% Saves the beginning location of the discharge
                    Discharge_Record(s,4) = Discharge_Record(s,2) + Discharge_Record(s,3);% Saves the ending location of the discharge
                    Discharge_Record(s,5) = Peak_Data(s,1);             % Saves the date/time associated with the beginning of the discharge
                    Spike_Tally = 0;                                    % Resets the spike counter to 0 for the next discharge
                    Discharge_Length = 0;                               % Resets the discharge length to 0 for the next discharger
                end
            end
            
            w = size(Discharge_Record,1);                   % Determines the number of discharges saved
            x = false(w,1);                                 % Logic statement placeholder variable
            for y=1:w
                if Discharge_Record(y,2) == 0               % Removes single spikes that aren't part of a discharge
                    x(y) = true;                            % Logic statement for removing discharges above
                end
            end
            Discharge_Record(x,:) = [];                     % Saves Discharge_Record variable with single spikes removed
            
            g = size(Discharge_Record,1);                   % Determines the number of discharges saved
            h = true(g,1);                                  % Logic statement placeholder variable
            for z=1:g                                       % Loop removes discharges with less than 3 spikes, less than 4 seconds, and greater than 40 seconds
                if Discharge_Record(z,1) > 3 && Discharge_Record(z,2) > 4 && Discharge_Record(z,2) < 40 % '3' = # of spikes in discharge, '4' = min discharge length, '40' = max discharge length
                    h(z) = false;                           % Logic statement for above loop
                end
            end
            Discharge_Record(h,:) = [];                     % Svaes Discharge_Record variable with ONLY TRUE discharges
...
% Plotting EEG Data - Seconds Time Scale -----------------------------------------------------------------------
            ...
            for t = 1:length(Discharge_Record(:,1))         % Loop that plots discharge bars on the figure
                plot([Discharge_Record(t,3) Discharge_Record(t,4)],[1.0 1.0], 'r', 'LineWidth', 4)  % Plots red bars above true discharges on plot
            end
            ...
            % Unsuppress the next two lines of code if you want figures to save automatically as MATLAB .fig files 
            %figurename = (strcat(Mouse_Number," ",'EEG Peaks and Discharges - Seconds.fig'));
            %savefig(figurename)
...
% Plotting EEG Data - Minutes Time Sacle ---------------------------------------------------------------------
            ...
            for t = 1:length(Discharge_Record(:,1))     % Loop that plots discharge bars on the figure
                plot([Discharge_Record(t,3)/60 Discharge_Record(t,4)/60],[1.0 1.0], 'r', 'LineWidth', 4)    % Plots red bars above true discharges on plot
            end
            yline(Lower_Threshold/1000, 'r-')           % Creates a red line at the lower threshold value (positive)
            yline(-Lower_Threshold/1000, 'r-')          % Creates a red line at the lower threshold value (negative)
            yline(Baseline/1000, 'b-')                  % Creates a blue line at the baseline value (positive)
            ...
            % Unsuppress the next two lines of code if you want figures to save automatically 
            %figurename = (strcat(Mouse_Number," ",'EEG Peaks and Discharges - Minutes.fig'));
            %savefig(figurename)
                      
%Export Discharge Data to Excel --------------------------------------------------------------------
            filename = strcat(Mouse_Number,' EEG Discharge Data.xlsx');     % Generates the name of the excel file using mouse number
            varNames = {'Date','Start Time (mins)','End Time (mins)','# of Spikes in Discharge','Duration (s)'}; % Saves names for variables on excel sheet
            Date_Time_3 = datetime(Discharge_Record(:,5),'ConvertFrom','datenum');  % Converts from datenum to datetime format
            T_pos = table(Date_Time_3,Discharge_Record(:,3)/60,Discharge_Record(:,4)/60,Discharge_Record(:,1),Discharge_Record(:,2),'VariableNames',varNames);  % Creates a table of data for excel
            writetable(T_pos,filename,'Sheet',1,'Range','B2')   % Exports data and creates excel file with discharge data
                       
% Discharge Counts and Output -----------------------------------------------------------------------
            discharge_count = length(Discharge_Record(:,1));    % Counts the number of true discharges
            fprintf('\nThe total number of EEG spikes is %0.f during the EEG trace\n', spike_count) % Outputs number of spikes to Command Window
            fprintf('The total number of discharges is %0.f during the EEG trace\n', discharge_count)   % Outputs number of discharges to Command Window
            fprintf('\nEEG analysis complete. \nDischarge data exported to excel\n')    % Tells user that the EEG data has been analyzed
            fprintf('-----------------------------------------------------------------------------------------------------------')
            ...
            Summary_Data(f,2) = discharge_count;    % Saves the number of discharges for each mouse analyzed
            Summary_Data(f,3) = Lower_Threshold;    % Saves the lower threshold value determined for each mouse
            
            % Discharge Count by Hour Export to Excel
            ...
            T_out_2 = table(datetime([c,zeros(size(c,1),2)],'f','MM-dd-uuuu HH:00'),accumarray(jj,1),'v',{'Dates','# of Discharges'}); % Creates table of discharges by hour
            filename = strcat(Mouse_Number,' EEG Spike and Discharge Data by Hour.xlsx');     % Creates the name of the excel file using mouse number
            writetable(T_out_2,filename,'Sheet',2,'Range','B2')   % Exports data and creates excel file with discharge data by hour
    ...
    varNames_summary = {'Mouse','Spikes','Discharges','Lower Threshold (µV)'};  % Creates variable names for summary excel sheet
    ...
    %delete(findall(0)); % Deletes all figures from MEMORY, even if they dont show
        % Suppress the previous line (295) if you want figures to show, also refer to line 22
end